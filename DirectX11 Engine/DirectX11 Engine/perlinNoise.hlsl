...
static const int perm[NOISE_TABLE_SIZE] =
{
...
float fbm(float x, float y, float z, int octaves, float persistence, float lacunarity)
{
    float total = 0.0f;
...
{
    if (id.x >= noiseWidth || id.y >= noiseHeight)
        return;
    
    float worldX = (float) id.x / noiseScale;
    float worldZ = (float) id.y / noiseScale;
    
    float noiseValue = fbm(worldX, worldZ, 0.0f, octaves, persistence, lacunarity);
    
    noiseValue = noiseValue * 0.5f + 0.5f;
    
    outputTexture[id.xy] = noiseValue;
}
